...
{
    public partial class frmInvoice : MetroFramework.Controls.MetroUserControl
    {
        ...
        private void FrmInvoice_Load(object sender, System.EventArgs e)
        {
            try
            {
                ...
                foreach (DataRow dr in dt.Rows)
                {
                    ...
                    SqlDataAdapter sda1 = new SqlDataAdapter("select * from tblInvoice where Id = N'" + id + "'", conn);
                    ...
                    foreach (DataRow dr1 in dt1.Rows)
                    {
                        ...
                        metroGrid1.Rows[n].Cells[3].Value = dr1[6].ToString();
                    }
                }
        ...
        private void MetroButton1_Click(object sender, System.EventArgs e)
        {
            try
            {
                ...
                String str = "select * from tblInvoiceReport where (Date like '%" + Month + "%' AND Customer like N'%" + CustomerName + "%')";
        ...
        private void PrintDocument1_PrintPage(object sender, System.Drawing.Printing.PrintPageEventArgs e)
        {
            try
            {
                ...
                String str = "select * from tblInvoiceReport where (Date like '%" + Month + "%' AND Customer like N'%" + CustomerName + "%') order by Date";
                SqlDataAdapter sda = new SqlDataAdapter(str, conn);
                ...
                e.Graphics.DrawString("สรุปภาษีขายประจำเดือน " + Month, new Font(font, 14, FontStyle.Bold), Brushes.Black, rect, strformat);
                ...
                for (int i = 0; i < 24; i++)
                {
                    ...
                    e.Graphics.DrawString(String.Format("{0:0,0.00}", float.Parse(dt.Rows[(currentPage * 24) + i][13].ToString())), new Font(font, 12, FontStyle.Regular), Brushes.Black, re4, strformat);
                    e.Graphics.DrawString(String.Format("{0:0,0.00}", float.Parse(dt.Rows[(currentPage * 24) + i][14].ToString())), new Font(font, 12, FontStyle.Regular), Brushes.Black, re5, strformat);
                    e.Graphics.DrawString(String.Format("{0:0,0.00}", float.Parse(dt.Rows[(currentPage * 24) + i][15].ToString())), new Font(font, 12, FontStyle.Regular), Brushes.Black, re6, strformat);
                    sum1 = sum1 + float.Parse(dt.Rows[(currentPage * 24) + i][13].ToString());
                    sum2 = sum2 + float.Parse(dt.Rows[(currentPage * 24) + i][14].ToString());
                    sum3 = sum3 + float.Parse(dt.Rows[(currentPage * 24) + i][15].ToString());
                }
